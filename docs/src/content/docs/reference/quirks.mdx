...
When the pipeline encounters a key on an aspect, `classifyKeys`
(`nix/lib/aspects/fx/key-classification.nix`) sorts it into one of four
categories:

| Key matches | Category | What happens |
|---|---|---|
| `den.quirks` | Pipe key | Collected as pipe data |
| `den.classes` (or the target class) | Class key | Emitted as class module |
| A nested aspect (its value carries a class/pipe/structural key) | Nested key | Walked as a sub-aspect |
| None of the above | Unregistered class key | Falls through to class (backwards compat) |

Pipe keys are checked first, so a quirk name always wins over a same-named
class key — but the registries assert they never collide (see above).
...
post-pipeline instead of `wrapCollectedClasses` wrapping them as modules.
...
### `pipe.collectAll predicate`

Like `pipe.collect`, but harvests from every scope matching the predicate
regardless of parent — not just siblings. Use it to aggregate fleet-wide
data (e.g. every host's address) from a single scope.

```nix
pipe.collectAll ({ host, ... }: true)
```

Entity kind filtering and non-terminal chaining work the same as
`pipe.collect`. Both stages also compose with `pipe.withProvenance`.
