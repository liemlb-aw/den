...
Maintaining an open-source project can be overwhelming for a single person, this is why we try to share the duties over many committed individuals.
...
As someone committed to Den, feel free to take decisions on it, just communicate anything you consider important for others to know.
...
PRs automatically run our `just ci` tests.
...
- Run `GH Pages` action on main for [`/versions.json`](https://den.denful.dev/versions.json) to be regenerated.
