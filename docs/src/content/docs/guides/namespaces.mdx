...
[`nix/lib/namespace.nix`](https://github.com/denful/den/blob/main/nix/lib/namespace.nix)
</Aside>
...
The second argument to `inputs.den.namespace` accepts:
- A **boolean** (`true`/`false`) -- export to `flake.denful.<name>` or keep local.
- A **list of sources** to merge from upstream flakes. Each source's
  `flake.denful.<name>` is merged into `den.ful.<name>`. Include `true` in the
  list to also export the merged result, e.g. `[ inputs.team-config true ]`.

## Terser References with Angle Brackets

Once `__findFile` is enabled, a namespace path resolves through `<namespace/path>`:
...
See [Angle Brackets Syntax](/guides/angle-brackets/) for how to enable
`__findFile` and the full resolution rules.
