...
description: Create new classes via den.batteries.forward.
---

import { Aside } from '@astrojs/starlight/components';

<Aside title="Source" icon="github">
[`forward.nix`](https://github.com/denful/den/blob/main/modules/aspects/batteries/forward.nix) --
[`os-user.nix`](https://github.com/denful/den/blob/main/modules/aspects/batteries/os-user.nix) --
[`forward-alias-class.nix`](https://github.com/denful/den/blob/main/templates/ci/modules/public-api/forward-alias-class.nix) --
[`forward-from-custom-class.nix`](https://github.com/denful/den/blob/main/templates/ci/modules/public-api/forward-from-custom-class.nix) --
[`guarded-forward.nix`](https://github.com/denful/den/blob/main/templates/ci/modules/public-api/guarded-forward.nix)
</Aside>
...
`den.batteries.forward` creates a new class by forwarding its module contents
...
<Aside title="Lower-level alternative: den.classes + route">

The `forward` battery is the ergonomic way to build a custom class. Under the
hood, Den's own built-in classes (`os`, `user`, `homeManager`, `hjem`, `maid`)
register the class name with `den.classes.<name>` and move its content with a
`den.lib.policy.route` policy. You can do the same directly:
...
Note the route spec uses `path` (not `intoPath`). See the
[policy reference](/reference/policies/) for the full `route` spec.
...
The `user` class ([`os-user.nix`](https://github.com/denful/den/blob/main/modules/aspects/batteries/os-user.nix))
