...
Every PR modifying this template runs `nix flake check` against Den,
by default the `main` branch. The CI version matrix can be extended to
other releases (such as `latest`) or specific commits.
...
{ denTest, ... }:
{
  flake.tests.bogus = {
    test-something = denTest (
      { den, lib, igloo, tuxHm, ... }:
      {
        # replace <system> if you are reporting a bug on macOS
...
```console
ls den/templates/ci/modules/deadbugs/*.nix
```
...
move `bug.nix` into the `templates/ci/modules/deadbugs/` directory, following naming conventions for both the file and test-suite name.
